...
int PC2[48] = {
    31,0,16,15,31,6, 15,1,17,14,30,22,
    16,2,18,13,29,9, 0,3,19,12,28,25,
    24,4,20,11,27,1, 8,5,21,10,26,17,
    23,6,22,9,25,14, 7,7,23,8,24,30
};
...
void desFunction(char R[], char K[], char output[]) {
    char expandedR[49];
    char xorResult[49];
    char sBoxOutput[33];

    permute(R, E, 48, expandedR);
    xorOperation(expandedR, K, xorResult);
    sBoxSubstitution(xorResult, sBoxOutput);
    permute(sBoxOutput, P, 32, output);

    printf("\nExpanded R: %s", expandedR);
    printf("\nAfter XOR with K1: %s", xorResult);
    printf("\nAfter S-Box: %s", sBoxOutput);
    printf("\nAfter P Permutation: %s", output);
}

int main() {
    char encrypted64[65];
    char key56[57] = "11001101001101001101101101111101111101111110111110";
    ...
    printf("DES Single Round Decryption\n");

    printf("\nEnter 64-bit encrypted Round 1 input: ");
    scanf("%64s", encrypted64);

    if (strlen(encrypted64) != 64) {
        printf("Invalid input! Please enter exactly 64 bits.\n");
        return 0;
    }

    printf("\n56-bit key after PC-1:\n%s\n", key56);

    strncpy(L1, encrypted64, 32);
    L1[32] = '\0';

    strncpy(R1, encrypted64 + 32, 32);
    R1[32] = '\0';
    ...
    */
    ...
    desFunction(L1, K1, fOutput);

    xorOperation(R1, fOutput, L0);

    strcpy(decrypted64, L0);
    strcat(decrypted64, R0);

    printf("\n\nRecovered L0: %s", L0);
    printf("\nRecovered R0: %s", R0);

    printf("\n\nDecrypted 64-bit output after reversing Round 1:");
    printf("\n%s\n", decrypted64);
